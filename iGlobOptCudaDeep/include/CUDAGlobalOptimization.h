...
void sendDataToCuda_deep(double *inBox, int inRank, int inFunc, int numBoxes, int * workLen,double* mins, double funcMin)
{
	...
	CHECKED_CALL(cudaMalloc((void **)&dev_mins, numThreads*sizeof(double)*2));
    CHECKED_CALL(cudaEventCreate(&start));
    CHECKED_CALL(cudaEventCreate(&stop));
	...
	CHECKED_CALL(cudaEventRecord(start, 0));
	calculateLimitsOnCUDA_deep<<<GridSize, BLOCK_SIZE>>>(dev_inBox, inRank, inFunc,numBoxes,dev_workLen,dev_mins,funcMin);
    CHECKED_CALL(cudaGetLastError());
	...
	CHECKED_CALL(cudaMemcpy(mins, dev_mins, numBoxes*sizeof(double)*2, cudaMemcpyDeviceToHost));
...
void fnGetOptValueWithCUDA_deep(double *inBox, int inRank, int inNumBoxesSplitCoeff, double inEps, int inMaxIter, int inFunc, double *outBox, double*outMin, double *outEps,int *status)
{
	...
	double *boxesResult = new double[3];
	double *restBoxes = new double[inRank*2 + 3];
	double *tempRestBoxes = NULL;
	double *h = new double[inRank];
	int numNewBoxes = 0;
	int *workLen;
	...
	*status = 1;
	while((countIter < inMaxIter) && (curEps >= inEps))
	{
		curNumBoxesSplitCoeff = (int) (inNumBoxesSplitCoeff/pow(numRestBoxes,1.0/inRank)) + 2;
		numBoxes = pow((double) curNumBoxesSplitCoeff,inRank);
		boxes = new double[numRestBoxes*numBoxes*(inRank*2+3)*1024];
		workLen = new int[numRestBoxes*numBoxes];
		mins = new double[numRestBoxes*numBoxes*2];
		for(k = 0; k < numRestBoxes; k++)
		{
			for(i = 0; i < inRank; i++)
			{
				h[i] = (restBoxes[k*(2*inRank + 3)+i*2 + 1] - restBoxes[k*(2*inRank + 3)+i*2])/curNumBoxesSplitCoeff;
			}
			...
			for(n = 0; n < numBoxes; n++)
			{
				...
				{
					index = ((n % numBoxes) % (long) pow((double)curNumBoxesSplitCoeff,i+1))/((long)pow((double)curNumBoxesSplitCoeff,i));
					boxes[(k*numBoxes + n)*(2*inRank+3)*1024+i*2] = restBoxes[k*(2*inRank + 3)+i*2] + h[i]*index;
					boxes[(k*numBoxes + n)*(2*inRank+3)*1024+i*2 + 1] = restBoxes[k*(2*inRank + 3)+i*2] + h[i]*(index+1);
				}
				workLen[k*numBoxes + n] = 1;
			}
		}

		sendDataToCuda_deep(boxes, inRank, inFunc, numRestBoxes*numBoxes, workLen,mins,funcMin);

		boxMinIndex = 0;

		int countBoxes = 0;
		int boxMinIndexInside = -1;
		int boxMinIndexOutside = -1;
		double minLimit = boxes[2*inRank];

		for(n = 0; n < numRestBoxes*numBoxes; n++)
		{
			countBoxes +=  workLen[n];
			if(funcMin > mins[2*n] && mins[2*n+1] >= 0) {funcMin = mins[2*n];boxMinIndexInside = mins[n*2+1];boxMinIndexOutside = n;}
			if(minLimit > boxes[n*1024*(2*inRank+3) + 2*inRank]) {minLimit = boxes[n*1024*(2*inRank+3) + 2*inRank];}
		}


		curEps = std::abs(minLimit - funcMin);
		*outEps = curEps;
		*outMin = funcMin;
		//memcpy(outBox,boxes + (boxMinIndexOutside*1024+boxMinIndexInside)*(2*inRank+3),inRank*2*sizeof(double));
		if(curEps < inEps)
		{
			*status = 0;
			delete [] restBoxes;
		...
		delete [] restBoxes;
		restBoxes = new double[countBoxes*(2*inRank+3)];

		j = 0;
		for(n = 0; n < numRestBoxes*numBoxes; n++)
		{
			for(i = 0; i < workLen[n]; i++)
			{
				if(funcMin >= boxes[(n*1024+i)*(2*inRank+3) + 2*inRank]) {
					for(k=0; k < inRank; k++)
					{
						restBoxes[j*(2*inRank+3) + 2*k] = boxes[(n*1024+i)*(2*inRank+3) + 2*k];
						restBoxes[j*(2*inRank+3) + 2*k+1] = boxes[(n*1024+i)*(2*inRank+3) + 2*k+1];
					}
					restBoxes[j*(2*inRank+3) + 2*inRank] = boxes[(n*1024+i)*(2*inRank+3) + 2*inRank];
					restBoxes[j*(2*inRank+3) + 2*inRank+1] = boxes[(n*1024+i)*(2*inRank+3) + 2*inRank+1];
					restBoxes[j*(2*inRank+3) + 2*inRank+2] = boxes[(n*1024+i)*(2*inRank+3) + 2*inRank+2];
					j++;
				}
			}
		}
