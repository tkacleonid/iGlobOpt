...
__global__ void globOptCUDA_1(double *inBox, int inDim, int *workLen, double *min, double inRec, double inEps, long long *workCounts);


void fnGetOptValueWithCUDA(double *inBox, const int inDim, const double inEps, double *outBox, double*outMin, int *status, double funRecord, char* filename)
{
	...
	*status = 1;
	...
	for(i = 0; i < MAX_NUM_RUNS ; i++)
	{
		...
		globOptCUDA_1<<<GridSize, BLOCK_SIZE>>>(dev_inBox,inDim,dev_workLen,dev_mins,funcMin,inEps,dev_workCounts);
